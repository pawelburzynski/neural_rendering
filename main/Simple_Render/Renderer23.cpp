...
double Renderer23::angle(QVector4D pos) {
...
double Renderer23::angle(QVector3D pos) {
    double s = asin(pos.y() / sqrt(pos.x()*pos.x()+pos.y()*pos.y()));
    if (pos.x() > - 0.0001) {
        return s + M_PI/2.0;
    } else {
...
double Renderer23::dist(QVector4D p1, QVector4D p2) {
   return sqrt((p1.x()-p2.x())*(p1.x()-p2.x())+(p1.y()-p2.y())*(p1.y()-p2.y())+(p1.z()-p2.z())*(p1.z()-p2.z()));
}


Renderer23::Renderer23() :
	viewWidth(0), viewHeight(0), training_dataPoints(0)
{
    initOpenCL();
}

void Renderer23::prepareCamPosArr()
{
...
void Renderer23::readImages(QString dir,  std::vector<QVector4D>* w_cam, QStringList* data_files) {
...
// Read data
void Renderer23::readData(const char *data_dir)
{
    ...
    for (int i = 0; i < training_data.size(); i++) {
        ...
        if (f.suffix() == "png") {
            ...
            qDebug() << "loaded: " << camera_index+1 << ", angle: " << angle(w_cam_training[camera_index]);
        }
    }
...
void Renderer23::updateViewSize(int newWidth, int newHeight)
{
